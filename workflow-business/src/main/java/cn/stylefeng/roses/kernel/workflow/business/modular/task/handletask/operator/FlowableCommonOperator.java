package cn.stylefeng.roses.kernel.workflow.business.modular.task.handletask.operator;
...
public class FlowableCommonOperator {
    ...
    public void addComment(String taskId, String comment) {
        ...
        resultComment.setUserId(Convert.toStr(LoginContext.me().getLoginUser().getUserId()));
        //记录审批操作
    ...
    public void addCommentAndCompleteTask(String taskId, String assignee, String comment) {
        ...
        if (ObjectUtil.isEmpty(assignee)) {
            ...
            Long loginUserId = LoginContext.me().getLoginUser().getUserId();
            String name = LoginContext.me().getLoginUser().getSimpleUserInfo().getRealName();
    ...
    public void smartCompleteNext(String processInstanceId) {
        ...
        if (ObjectUtil.isNotEmpty(nextTaskList)) {
            ...
            if (nextTaskList.size() == 1) {
                ...
                //当前登录用户
                String sysLoginUserId = Convert.toStr(LoginContext.me().getLoginUser().getUserId());
                //任务有指定审批人才去校验
