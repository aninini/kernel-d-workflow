...
flowableModeler
  ...
  .config(['$provide', '$routeProvider', '$selectProvider', '$translateProvider', function ($provide, $routeProvider, $selectProvider, $translateProvider) {
    ...
    $routeProvider
        ...
        if (FLOWABLE.CONFIG.appDefaultRoute) {
            ...
            //编辑模型界面-fengshuonan 202008310023
            var query = window.location.search.substring(1);
            var modelIdAndStr = query.split("modelId=");
            if(modelIdAndStr === null || modelIdAndStr === undefined || modelIdAndStr === ""){
                redirectTo:  "403.html"
            }else{
                var modelId = modelIdAndStr[1].split("&")[0];
                $routeProvider.when('/', {
  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$http', '$window', 'appResourceRoot',
        function($rootScope, $timeout, $modal, $translate, $location, $http, $window, appResourceRoot) {
            ...
            $rootScope.getUserInfo = function (token) {
                $http.get(FLOWABLE.APP_URL.getAccountUrl(token))
            ...
            /* 此处获取token并校验，然后获取用户信息 */
            var token = $rootScope.getQueryVariable("token");
            if(token === null || token === undefined || token === ""){
                alert("请携带token访问本页面");
                location.href = "403.html"
            } else {
                var theme = $rootScope.getQueryVariable("theme");
                if(theme !== null && theme !== undefined) {
                    DynamciLoadUtil.loadCSS("styles/theme/" + theme + ".css")
                }
