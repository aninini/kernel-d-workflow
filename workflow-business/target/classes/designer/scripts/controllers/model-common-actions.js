    	...
    	$scope.ok = function () {
    		...
    		$http({method: 'PUT', url: FLOWABLE.APP_URL.getModelUrl($scope.popup.id), data: updateData}).
    			success(function(data, status, headers, config) {
    				...
    				$scope.addAlertPromise($translate(popupType + '.ALERT.EDIT-CONFIRM'), 'info');
    				$scope.$hide();
...
//发布模型 fengshuonan
angular.module('flowableModeler')
    .controller('PublicModelPopupCtrl', ['$rootScope', '$scope', '$http', '$translate', '$location',
        function ($rootScope, $scope, $http, $translate, $location) {
            ...
            if ($scope.model.process) {
                model = $scope.model.process;
                popupType = 'PROCESS';
                publicUrl = FLOWABLE.APP_URL.getModelPublicUrl();
            } else if ($scope.model.caseModel) {
                model = $scope.model.caseModel;
                popupType = 'CASE';
            } else if ($scope.model.form) {
            ...
            //开始加载分类 fengshuonan
            $http({method: 'GET', url: FLOWABLE.APP_URL.getCategoryUrl(), headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json; charset=UTF-8',
                    'Authorization': 'Bearer ' + $rootScope.getQueryVariable("token")
                }}).
            success(function(data, status, headers, config) {
                if(data.success) {
                    $scope.popup.modelCategory = data.data;
                } else {
                    $scope.popup.loading = false;
                    $scope.popup.errorMessage = data.message;
                }
            ...
            $scope.ok = function () {
                if (!$scope.popup.modelName || $scope.popup.modelName.length == 0 ||
                    !$scope.popup.modelKey || $scope.popup.modelKey.length == 0 ||
                    !$scope.popup.selectedCategory || $scope.popup.selectedCategory.length == 0) {
                ...
                $scope.model.key = $scope.popup.modelKey;
                $scope.model.description = $scope.popup.modelDescription;
                $scope.model.selectedCategory = $scope.popup.selectedCategory;

                $scope.popup.loading = true;
                var publicData = {
                    modelId: $scope.popup.id,
                    category: $scope.model.selectedCategory.code
                };

                $http({method: 'POST', url: publicUrl, data: publicData, headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json; charset=UTF-8',
                        'Authorization': 'Bearer ' + $rootScope.getQueryVariable("token")
                    }}).
                success(function(data, status, headers, config) {
                    if(data.success) {
                        $scope.addAlertPromise($translate(popupType + '.ALERT.PUBLIC-CONFIRM'), 'info');
                        $scope.$hide();
                        $scope.popup.loading = false;
